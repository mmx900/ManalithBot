					...
					while ( array.getElement(i).getTokenSubtype() != TokenSubtype.TrClose )
					{
						if ( array.getElement(i++).getTokenSubtype() == TokenSubtype.TdOpen )
						{
							while ( array.getElement(i).getTokenSubtype() != TokenSubtype.TdClose )
							{
								...
								{
									if ( array.getElement(i++).getTokenString().contains("buildinfo") ) 
										bString = true;
									
									while ( array.getElement(i).getTokenSubtype() != TokenSubtype.AClose )
									{
										if ( array.getElement(i).getTokenSubtype() == TokenSubtype.TextString && bString == true )
										{
											...
											{
												if  ( ( versionstrings[l].charAt(0) >= '0' && versionstrings[l].charAt(0) <= '9' )
													|| ( versionstrings[l].charAt(0) == 'b' || versionstrings[l].charAt(0) == 'a' )
													|| versionstrings[l].charAt(0) == 'p' )
												{
													if ( !value.equals("") ) value += "-" + versionstrings[l];
													else
													{
														if ( versionstrings[l].contains(".") )
