...
public class SebeolAutomataEngine extends InputSequenceAutomataEngine {
	...
	protected boolean isISingleConsonant(String tICon) {
		try {
			...
			SebeolSymbol.SebeolISingleConsonant check1 = SebeolSymbol.SebeolISingleConsonant
					.valueOf(tICon);
			return true;
		} catch (Exception e) {
			return false;
	...
	protected boolean isIDoubleConsonant(String tICon) {
		try {
			@SuppressWarnings("unused")
			SebeolSymbol.SebeolIDoubleConsonant check1 = SebeolSymbol.SebeolIDoubleConsonant
					.valueOf(tICon);
			return true;
		} catch (Exception e) {
			return false;
	...
	protected boolean isVowel(String tVow) {
		try {
			@SuppressWarnings("unused")
			SebeolSymbol.SebeolVowel check1 = SebeolSymbol.SebeolVowel
					.valueOf(tVow);
			return true;
		} catch (Exception e) {
			return false;
	...
	protected boolean isFConsonant(String tFCon) {
		try {
			@SuppressWarnings("unused")
			SebeolSymbol.SebeolFConsonant check1 = SebeolSymbol.SebeolFConsonant
					.valueOf(tFCon);
			return true;
		} catch (Exception e) {
			return false;
	...
	private boolean isSpecialChar(String tChar) {
		try {
			@SuppressWarnings("unused")
			SebeolSymbol.SebeolSpecialChar check1 = SebeolSymbol.SebeolSpecialChar
					.valueOf(tChar);
			return true;
		} catch (Exception e) {
			return false;
	...
	private String inputIConsonant(boolean isLastPosition, String token)
		...
		// 초성 유무 상태 저장
		boolean beingIConsonant = (syl.getIConsonantValue() != SebeolSymbol.SebeolIConsonant.nul
				.value());
		...
		if (!(!isLastPosition && !beingIConsonant)) {
			...
			// 초성은 있고 중성이 없다?!
				...
				SebeolSymbol.SebeolIConsonant init = SebeolSymbol.SebeolIConsonant
						.valueOf(syl.getIConsonantKeySymbol());
				SebeolSymbol.SebeolFConsonant fin = SebeolSymbol.SebeolFConsonant
	...
	private String inputVowel(boolean isLastPosition, String token)
		...
		// 중성 유무 상태 저장
		boolean beingVowel = (syl.getVowelValue() != SebeolSymbol.SebeolVowel.nul
				.value());
		...
		if (!(!isLastPosition && !beingVowel)) {
			...
			// 자음이 없어서 완성체가 아닌 경우
				...
				SebeolSymbol.SebeolVowel vow = SebeolSymbol.SebeolVowel
	...
	private String inputFConsonant(boolean isLastPosition, String token)
		...
		boolean beingFConsonant = (syl.getFConsonantValue() != SebeolSymbol.SebeolFConsonant.nul
				.value());
	...
	private String inputSpecialChar(String token)
		...
		SebeolSymbol.SebeolSpecialChar spec = SebeolSymbol.SebeolSpecialChar
		...
		// 자음이나 모음이 없다.
			...
			SebeolSymbol.SebeolIConsonant init = SebeolSymbol.SebeolIConsonant
					.valueOf(syl.getIConsonantKeySymbol());
			SebeolSymbol.SebeolVowel vow = SebeolSymbol.SebeolVowel.valueOf(syl
					.getVowelKeySymbol());
			SebeolSymbol.SebeolFConsonant fin = SebeolSymbol.SebeolFConsonant
					.valueOf(syl.getFConsonantKeySymbol());
	...
	private String inputOtherChar(String token)
			throws LayoutNotSpecifiedException {
		String result = "";
