...
import org.manalith.ircbot.plugin.newCER.Exceptions.EmptyTokenStreamException;
...
public class CERInfoProvider {
	...
	public String commandInterpreter ( ) throws EmptyTokenStreamException, InvalidArgumentException, IOException, SQLException, ClassNotFoundException
	{
		String result = "";
	...
	private void showHelpMessage ( )
	{
		...
		System.out.println ( "\t--show [Currency_unit] ([Field]) : 지정한 화폐단위에 대한 환율 항목을 보여줍니다.");
		System.out.println ( "\t--convert [Amount] [Currency_unit] : 지정 금액을 원화로 취급하며 [Currency_Unit] 으로 환산하여 보여줍니다.");
		System.out.println ( "\t--convert [Amount] [Curr_unit1] [Curr_unit2] : 지정 금액을 [Curr_unit1] 단위로 취급하며 [Curr_Unit2] 로 환산하여 보여줍니다.");
		System.out.println ( "\t--convert [Curr_unit1] [Curr_unit2] : 금액 1을 [Curr_unit1] 단위로 취급하며 [Curr_unit2] 로 환산하여 보여줍니다.");
		System.out.println ( "\t--buycash [Currency_unit] [Amount] : 환전 거래시 [Currency_name]단위의 [Amount] 금액으로 바꾸기 위해 필요한 원화금액을 보여줍니다.");
		System.out.println ( "\t--cellcash [Currency_unit] [Amount] : 환전 거래시 [Currency_name]단위의 [Amount] 금액을 받을 수 있는 원화금액을 보여줍니다." );
		System.out.println ( "\t--sendremit [Currency_unit] [Amount] : 송금 보낼 때 [Currency_name]단위의 [Amount] 금액을 보내기 위해 필요한 원화금액을 보여줍니다." );
		System.out.println ( "\t--recvremit [Currency_unit] [Amount] : 송금 받을 때 [Currency_name]단위의 [Amount] 금액에 대해 받을 수 있는 원화금액을 보여줍니다.\n" );
	...
	*/
	...
	private String convert ( String value, String CurrencyUnit1, String CurrencyUnit2 ) throws SQLException, ClassNotFoundException
	{
		String result = "";
		
		if ( CurrencyUnit1.equals(CurrencyUnit2) )
		{
			result = value + CurrencyUnit1 + " => " + value + " " + CurrencyUnit2;
			return result;
		}
		...
		{			
			...
			resval = String.format("%.2f", ( s_value / central_rate ) * currency_unit );
		...
		else if ( CurrencyUnit2.equals("KRW") )
		{
			...
			resval = String.format("%.2f", ( s_value / currency_unit ) * central_rate );
		...
		result  = value + " " + CurrencyUnit1 + " => " + resval + " " + CurrencyUnit2;
		return result;
	}
