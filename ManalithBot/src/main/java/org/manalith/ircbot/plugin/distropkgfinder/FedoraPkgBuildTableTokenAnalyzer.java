...
public class FedoraPkgBuildTableTokenAnalyzer extends TokenAnalyzer {
	...
	public TokenType getTokenType(String TokenString) {
		...
		Pattern table_pattern = Pattern.compile("\\<[\\/]?table((\\s)(\\S)+\\=\\\"(\\s|\\S)+\\\")*\\>");
		Pattern img_pattern = Pattern.compile("\\<img((\\s)(\\S)\\=\\\"(\\s|\\S)+\\\")*\\/\\>");
		Pattern th_pattern = Pattern.compile("\\<[\\/]?th((\\s)(\\S)+\\=\\\"(\\s|\\S)+\\\")*\\>");
	...
	public TokenArray analysisTokenStream() throws EmptyTokenStreamException {
		...
		while ( i < len )
		{
			...
			else
			{
				...
				{
					String [] tagnattr = tokenString.substring(1,tokenString.length() - 1).split("\\s");
					if ( tagnattr.length != 1 )
					{
						for ( int l = 1 ; l < tagnattr.length ; l++ )
						{
							if ( tagnattr[l].contains("row-odd") || tagnattr[l].contains("row-even") )
							{
				...
				else if ( currentTokenSubtype == TokenSubtype.TrClose && inBoundOfResult )
				{
