...
public class KVLTableTokenAnalyzer extends TokenAnalyzer {
	...
	public TokenSubtype getTokenSubtype(String tokenString,
		...
		/*
		 * else if ( hashCode == TokenType.STRONG.hashCode() ) { if (
		 * tokenString.charAt(1) == '/') result = TokenSubtype.StrongClose; else
			...
			if (currentTokenType == TokenType.Unknown) {
				...
				if (currentTokenSubtype == TokenSubtype.TableOpen) {
					if (tokenString.length() != 7) {
						// <table ...>
						String[] CheckTableOption = tokenString.substring(1,
						...
						for (int l = 1; l < CheckTableOption.length; l++) {
							...
							if (KeyVal[0].equals("class")
									&& KeyVal[1].substring(1,
											KeyVal[1].length() - 1).equals(
											"kver")) {
								// class="kval"
								inBoundOfTable = true;
							}
