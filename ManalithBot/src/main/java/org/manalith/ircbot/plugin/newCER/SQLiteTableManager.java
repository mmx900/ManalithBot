...
public class SQLiteTableManager {
	...
	public void insertDataToTable ( ) throws SQLException
	{
		...
		try
		{
			Elements trs = Jsoup.connect(url).get().select("div.tbl_area").get(0).select("table.tbl_exchange").get(0).select("tbody").get(0).select("tr");
			int size = trs.size();
			for ( int i = 0 ; i < size ; i++ )
			{
				for ( int j = 0 ; j < 8 ; j++ )
				{
					...
					if ( j == 0 )
					{
						...
						String country_name = "";
						while ( !Pattern.compile("[A-Z]{3}").matcher(split[k]).matches() )
						{
					...
					{
						if ( Double.toString(Double.parseDouble(tds.get(j).text().replaceAll( "\\,", "" ))).equals("0.0") )
						{
							j += 2;
							for ( int k = 0 ; k < 3 ; k++ )
							{
						...
						else
						{
							for ( int k = 0 ; k < 3 ; k++ )
							{
								stmt.setDouble(7 + k, Double.parseDouble(tds.get(j++).text().replaceAll( "\\,", "" )));
							}
							j--; // trick
						}
					}
					...
					{
						stmt.setDouble(3 + j, Double.parseDouble(tds.get(j).text().replaceAll( "\\,", "" )));
					}
				}
