...
private TokenArray array;
	...
	public String getPkgVersion ( ) throws EmptyTokenStreamException
	{
		...
		try
		{
			while ( i < cnt )
			{
				if ( array.getElement(i).getTokenSubtype() == TokenSubtype.POpen )
				{
					...
					{
						if ( array.getElement(i).getTokenString().equals("Contents:") )
						{
							i++;
											...
											while ( array.getElement(i).getTokenSubtype() != TokenSubtype.LiClose )
											{
												if ( array.getElement(i++).getTokenSubtype() == TokenSubtype.AOpen )
												{
													while ( array.getElement(i).getTokenSubtype() != TokenSubtype.AClose )
													{
														...
														{
															String [] DistVersion = array.getElement(i).getTokenString().split("\\s");
															
															if ( version == 0 )
															{
																try
																{
																	version = Integer.parseInt(DistVersion[1]);
																}
																catch ( Exception e )
																{
																	i++;
				...
				else if ( array.getElement(i).getTokenSubtype() == TokenSubtype.UlOpen )
				{
					...
					tagnattr = temp.substring(1,temp.length() - 1).split("\\s");
								...
								if ( array.getElement(i++).getTokenSubtype() == TokenSubtype.LiOpen )
								{
									while ( array.getElement(i).getTokenSubtype() != TokenSubtype.LiClose )
									{
										if ( array.getElement(i).getTokenSubtype() == TokenSubtype.AOpen )
										{
											...
											for ( int l = 1 ; l < len ; l++ )
											{
												keyval = tagnattr[l].split("\\=");
												if ( keyval[0].equals("class") )
												{
													str = keyval[1].substring(1, keyval[1].length() - 1);
												}
												else if ( keyval[0].equals("href") && str.equals("koji") )
												{
													String tmp = "";
													for ( int ll = 1 ; ll < keyval.length ; ll++ )
													{
														if ( !tmp.equals("") ) tmp += "=" + keyval[ll];
														else tmp += keyval[ll];
													}
													...
													FedoraPkgBuildTableTokenAnalyzer analyzer = new FedoraPkgBuildTableTokenAnalyzer ( data );
													TokenArray arr = analyzer.analysisTokenStream();
													FedoraPkgBuildTableLatestVersionParser parser = new FedoraPkgBuildTableLatestVersionParser ( arr );
													pkgversion = parser.getPkgVersion();
		...
		{
			if ( pkgversion[j] == null ) break;
			String [] pkgndist = pkgversion[j].split("fc");
			pkgndist[0] = pkgndist[0].substring(0,pkgndist[0].length() - 1); // remove dot.
			if ( version != 0 )
			{
