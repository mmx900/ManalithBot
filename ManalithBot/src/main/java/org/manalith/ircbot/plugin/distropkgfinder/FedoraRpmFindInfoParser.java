						...
						while( array.getElement(i).getTokenSubtype() != TokenSubtype.TBodyClose )
						{
							if ( array.getElement(i++).getTokenSubtype() == TokenSubtype.TrOpen )
							{
								while ( array.getElement(i).getTokenSubtype() != TokenSubtype.TrClose )
								{
									if ( !isTheFirstRow )
									{
										while ( array.getElement(i).getTokenSubtype() != TokenSubtype.TrClose ) i++;
										isTheFirstRow = true;
									}
									else 
									{
										if ( array.getElement(i++).getTokenSubtype() == TokenSubtype.TdOpen )
										{
											while ( array.getElement(i).getTokenSubtype() != TokenSubtype.TdClose )
											{
												...
												{
													case 0:
														temp = array.getElement(i).getTokenString();
														colCount++;
														break;
													case 1:
														Description = array.getElement(i).getTokenString();
														colCount++;
														break;
													case 2:
														Dist = array.getElement(i).getTokenString();
														colCount++;
														break;
													case 3:
														colCount++;
												...
												if ( Dist.contains("Fedora") && colCount == 4 )
												{
													String [] tmp = Dist.split("\\s");
													int DistVersion = 0;
													int startIndex = 0;
													
													try
													{
														DistVersion = Integer.parseInt(tmp[1]);
													}
													catch ( Exception e )
													{
														;
													...
													tmp = temp.split("\\.");
													int k = 1;
													while ( !tmp[k].contains("fc") )
													{
														tmp[startIndex] += ( "." + tmp[k++] );
													}
													
													temp = tmp[0]; // package name and version.
													
													tmp = temp.split("\\-");
													
													k = 1;
													while ( !( tmp[k].charAt(0) >= '0' && tmp[k].charAt(0) <= '9') )
													{
														tmp[startIndex] += ("-" + tmp[k++] );
													}
